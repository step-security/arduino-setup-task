...
async function verifyChecksum(
  ...
  try {
    core.debug(`Downloading checksums from ${checksumUrl}`);
    ...
    const entry = response.data
      ...
      .find((parts) => parts.length === 2 && parts[1] === fileName);
    if (!entry) {
      core.warning(
        `Checksum for ${fileName} not found in release. Skipping verification.`,
      );
      return;
    }
    [expectedHash] = entry;
    ...
    core.warning(
      `Unable to download checksums file. Skipping checksum verification.`,
    );
    return;
  }

  const computedHash = await computeFileHash(downloadPath);
  if (computedHash.toLowerCase() !== expectedHash.toLowerCase()) {
    throw new Error(
      `Checksum verification failed for ${fileName}.\nExpected: ${expectedHash}\nGot:      ${computedHash}`,
    );
  }
  core.debug(`Checksum verified for ${fileName}: ${computedHash}`);
}
...
async function downloadRelease(version: string): Promise<string> {
  ...
  await verifyChecksum(downloadPath, version, fileName);
